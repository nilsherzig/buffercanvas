...
function addCursorMovementListener(editorId, editor) {
  editor.on("keydown", function(cm, event) {
    if (cm.getOption("keyMap").startsWith("vim") && cm.state.vim?.insertMode) {
      return; // Nichts tun, wenn im Vim-Einfügemodus
    }
    ...
    if ((keyCode === 74 || keyCode === 40) && cursor.line === lineCount - 1) {
      // J oder Pfeil nach unten
      event.preventDefault();
      focusNextEditor(editorId);
    } else if ((keyCode === 75 || keyCode === 38) && cursor.line === 0) {
      // K oder Pfeil nach oben
      event.preventDefault();
      focusPreviousEditor(editorId);
    } else if (ctrlKey && keyCode === 85) {
      // Ctrl + U
      event.preventDefault();
      focusPreviousEditor(editorId);
...
function focusNextEditor(currentEditorId) {
  ...
  if (currentIndex >= 0 && currentIndex < editorIds.length - 1) {
    nextEditor = editors[editorIds[currentIndex + 1]]
    const x = window.scrollX;
    const y = window.scrollY;
    nextEditor.focus({ preventScroll: true });
    window.scrollTo(x, y);
    centerCursor(nextEditor);
  }
}
...
function focusPreviousEditor(currentEditorId) {
  ...
  if (currentIndex > 0) {
    ...
    prevEditor.setCursor({ line: lastLine, ch: 0 });
...
function selectLanguage(editor) {
  const language = prompt(
    "Bitte Sprache eingeben (z.B. 'javascript', 'python'):",
  );
  if (language) {
